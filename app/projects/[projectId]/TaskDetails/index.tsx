...
export const TaskDetails = () => {
  ...
  const { statuses, labels } = useProjectQueries(projectId);
  const { members } = useProjectQueries(projectId) as {
    members?: IUser[];
  ...
  const allMembers = useMemo(() => {
    ...
    const isCurrentUserMember = members.some((member) => member.id === user.id);
